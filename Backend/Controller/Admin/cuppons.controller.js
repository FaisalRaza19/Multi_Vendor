...
const createCoupon = async (req, res) => {
    try {
        const { shopId, couponName, value, expirationDate, products } = req.body;

        if (!shopId || !couponName || !value || !expirationDate || !products) {
            return res.status(400).json({ message: "All fields are required." });
        }
        ...
        if (products === "All") {
            productsToUpdate = shop.products;
        } else if (Array.isArray(products)) {
            productsToUpdate = shop.products.filter(prod => products.includes(prod._id.toString()));
            const invalidProducts = products.filter(prodId => !shop.products.some(prod => prod._id.toString() === prodId));
            if (invalidProducts.length > 0) {
                return res.status(404).json({ message: "Some product IDs are invalid.", invalidProducts });
            }
        } else {
        ...
        if (couponCreated) {
            await shop.save();
            return res.status(201).json({ message: "Coupons added successfully.", updatedProducts: productsToUpdate });
        } else {
            return res.status(400).json({ message: "No coupons were created or products already have coupons." });
        }
    } catch (error) {
        res.status(500).json({ message: "Internal server error.", error: error.message });
    }
