...
import { fetchProducts } from "../../../../Redux/Redux Store/products&Events.jsx";
        ...
        <div className="p-4">
            ...
            {status === "loading" ? (
                <div className="flex items-center justify-center h-52">
                    <AiOutlineLoading3Quarters size={50} className="text-green-500 animate-spin" />
                </div>
            ) : status === "succeeded" ? (
                <>
                        ...
                        <div ref={scrollContainerRef} className="flex space-x-4 overflow-hidden mb-6">
                            {items.map((e) => (
                                <div key={e?._id} className="min-w-[270px] flex-shrink-0 ml-7 border rounded p-2 shadow-md text-sm mb-6">
                                    <Link to={`/product/${e?._id}`} className='flex justify-center'>
                                        <img src={e.images[0]?.url || ""} alt="" className="w-40 h-40 object-cover rounded-lg mb-6" />
                                    </Link>
                                    <Link to={`/Shop/${e.shopInfo?.shopId}`}>
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        {e?.giveOffer ? (
                                            ...
                                            <span className="text-green-500 ml-4 font-bold">${e?.actualPrice || 0}</span>
                                        )}
                                    </div>
