...
const admin_VerifyEmail = ({isAuth}) => {
  const {shopVerify,adminResendCode } = useContext(ContextApi);
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      await shopVerify({ otp, navigate,isAuth});
      setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col py-8 sm:px-6 lg:px-8">
      <div className="flex ml-10 cursor-pointer" onClick={() => navigate("/Shop-register")}>
