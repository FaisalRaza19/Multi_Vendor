...
const profileEmailVerify = () => {
  ...
  const { VerifyProfile, ResendCode } = useContext(ContextApi).userAuth;
  ...
  const handleVerify = async () => {
    ...
    try {
      const data = await VerifyProfile({ otp: otp.join(''), navigate,});
      showAlert(data)
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-8">
      <button onClick={() => navigate('/user-dashboard')} className="self-start ml-8 mb-4">
        <FaArrowLeft size={24} />
      </button>
