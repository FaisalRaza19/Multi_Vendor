...
const adminProfile_EmailVerify = ({ isAuth }) => {
  const {showAlert} = useContext(ContextApi)
  const { verifyShopProfile, adminResendCode } = useContext(ContextApi).adminAuth;
  ...
  const handleVerify = async () => {
    ...
    try {
      const data = await verifyShopProfile({ otp: otp.join(''), navigate, isAuth });
      showAlert(data)
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-8">
      <button onClick={() => navigate('/signUp')} className="self-start ml-8 mb-4">
        <FaArrowLeft size={24} />
      </button>
...
export default adminProfile_EmailVerify;
