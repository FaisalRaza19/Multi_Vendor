...
const admin_Profile = () => {
  ...
  const { getShop, UpdateShopLogo, UpdateShopProfile } = useContext(ContextApi).adminAuth;
  ...
  const fetchShop = async () => {
    try {
      ...
      setUser(data.data.shop)
      setFormData(data.data.shop)
      setProfileImage(data.data.shop.shopLogo?.url || "/public/pic.jpg")
    } catch (error) {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (files && files[0]) {
      ...
      try {
        ...
        setLoading(true);
        setProfileImage(updatedAvatar.data);
