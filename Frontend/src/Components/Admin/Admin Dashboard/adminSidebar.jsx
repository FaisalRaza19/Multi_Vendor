...
const AdminSidebar = ({shopId}) => {
    ...
    return (
        <div className="fixed bg-white h-full">
            ...
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r transform transition-transform duration-200 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    } lg:translate-x-0 lg:static lg:z-0`}
            >
                <div className="flex items-center justify-between mt-7 p-4 border-b z-50 lg:hidden"></div>
                <nav className="p-4 space-y-3">
                    {sideItems.map((item) => {
                        const resolvedHref = item.href.replace(':shopId', shopId);
